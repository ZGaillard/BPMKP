    ...
    public LPSolution solve(LinearProgram lp, double timeLimitSeconds) {
        ...
        try {
            ...
            for (Variable var : lp.getVariables()) {
                double lb = var.lowerBound();
                double ub = var.upperBound();
                MPVariable mpVar;
                switch (var.type()) {
                    case BINARY -> mpVar = solver.makeBoolVar(var.name());
                    case INTEGER -> mpVar = solver.makeIntVar(lb, ub, var.name());
                    case CONTINUOUS -> mpVar = solver.makeNumVar(lb, ub, var.name());
                    default -> throw new IllegalArgumentException("Unknown variable type: " + var.type());
                }
