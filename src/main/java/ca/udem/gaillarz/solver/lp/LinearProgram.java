...
public class LinearProgram {
    ...
    public boolean isFeasible(Map<Variable, Double> values, double tolerance) {
        ...
        for (Variable v : variables) {
            ...
            if (val < v.lowerBound() - tolerance || val > v.upperBound() + tolerance) {
                return false;
            }
        }
        return true;
    ...
    public String toLPFormat() {
        ...
        for (Constraint c : constraints) {
            ...
            for (Map.Entry<Variable, Double> term : c.getCoefficients().entrySet()) {
                if (!first) sb.append(" + ");
                sb.append(term.getValue()).append(" ").append(term.getKey().name());
                first = false;
            }
            sb.append(" ");
        ...
        sb.append("Bounds\n");
        for (Variable v : variables) {
            sb.append(" ").append(v.lowerBound()).append(" <= ").append(v.name()).append(" <= ").append(v.upperBound()).append("\n");
        }
        sb.append("End\n");
        return sb.toString();
    }
